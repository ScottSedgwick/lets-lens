...
mapL k = Lens (\g s -> (\v -> insdel k v s) <$> g (Map.lookup k s))
    where 
        insdel k' Nothing   s' = Map.delete k' s'
        insdel k' (Just v') s' = Map.insert k' v' s'
...
setL k = Lens (\g s -> (insdel s) <$> g (Set.member k s))
    where 
        insdel s b = if b then Set.insert k s else Set.delete k s
...
compose (Lens stab) (Lens qrst) = Lens (\afb q -> qrst (stab afb) q)
...
product l1 l2 = Lens (\g (s,q) -> (\(b,d) -> (set l1 s b, set l2 q d)) <$> g (get l1 s, get l2 q))
...
choice l1 l2 = Lens res
    where 
        res a2fb (Left s)  = Left  <$> (set l1 s) <$> (a2fb (get l1 s))
        res a2fb (Right q) = Right <$> (set l2 q) <$> (a2fb (get l2 q))
...
setStreetOrState :: Either Person Locality -> String
